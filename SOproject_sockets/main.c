...
int socket_server(struct sockaddr_un channel_srv, char *socket_path);
...
int main(int argc, char *argv[]) {
    ...
    listenfd = socket_server(channel_srv, socket_path);

    get_time_useconds(&time_usec_begin);

    GENOME_LIST *gl = (GENOME_LIST *) calloc(1, sizeof(GENOME_LIST));

    read_genomes(gl, "/Users/andrebarbosa/CLionProjects/SOproject_sockets/input/cds.fna");
    ...
    int fd = open("/Users/andrebarbosa/CLionProjects/SOproject_sockets/result/sockets_mutations.txt",
    ...
    for (i = 0; i < nfilhos; i++) {
        ...
        if (pids[i] == 0) {
            ...
            if ((uds = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) // Criar server socket
            {
                perror("socket error");
                exit(1);
            }
            ...
            while (g != NULL) {
                ...
                save_mutation_array_socket(mutation_array, 0, uds);
    ...
    for(int k =0; k< nfilhos; k++) {
        ...
        while ((bytes = readn(connfd, buf, BUF_SIZE))) {
            writen(fd, buf, strlen(buf)); // write to socket (size_t)bytes
        }
        close(connfd);
    }
...
/**
 * Saves the mutation array to socket
 * @param mutation_array - array containing all the discovered mutations
 * @param detail - detail flag.
 * @param uds - socket
 * (0) outputs only the number of found mutations per genome / gene
 * (1) outputs all the found mutations (on a nucleotide level) per genome / gene
 */
void save_mutation_array_socket(MUTATION_ARRAY *mutation_array, int detail, int uds) {
    char *buf = (char *) malloc(sizeof(char) * 1000000);
